-- unokai-reloaded: a contrast-optimised overhaul of the built-in unokai theme
--
-- The stock unokai has multiple highlights that fail WCAG contrast minimums
-- against its #282923 background.  This file redefines every low-contrast
-- group while keeping the Monokai-inspired character intact.
--
-- ── Problems fixed ──────────────────────────────────────────────────
...
return {
  {
    ...
    config = function()
      ...
      -- second create_augroup(..., { clear = true }) would wipe this one.
      vim.api.nvim_create_autocmd("ColorScheme", {
        group = vim.api.nvim_create_augroup("UnokaiReloaded", { clear = true }),
        callback = function()
